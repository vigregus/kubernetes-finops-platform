...
`checkout` writes to CloudNativePG (Postgres), Redis, and Kafka (`orders-{prod,stage}` topics on one shared Strimzi cluster); `analytics` is split into an HTTP API and `analytics-worker`, a separate Deployment running only the Kafka consumer - so the queue can be scaled independently of request traffic and the cost of asynchronous processing is separable from the cost of serving. The worker writes each consumed event into `order_events`, its own CloudNativePG cluster, and reads/writes Redis. `checkout` also exposes `/checkout/lookup` - a real Postgres `SELECT` with three genuinely distinct outcomes (row found, valid-but-absent id, malformed id that Postgres itself rejects), not a simulated coin flip. A static storefront (`charts/frontend`) fronts both services through one path-routed ingress host, so traces can originate in a real browser tab instead of only from `k6`/`curl`.
...
Each leaf directory under `gitops/` holds one Argo CD `application.yaml` (plus a `values.yaml` when the source is a Helm chart). Local, chart-backed components (`charts/*`) hold only `Chart.yaml` and `templates/` — they are not self-contained for `helm template`/`helm install` outside Argo CD, since defaults are supplied entirely through the matching `gitops/**/values.yaml`.
...
- an Argo CD root application (`gitops/01-root`) that recursively discovers one `application.yaml` per component under `gitops/02-infra`, `gitops/03-finops`, and `gitops/04-business-app`,
...
6. Let Argo CD install GitOps-managed platform dependencies such as Kyverno, then reconcile each component `application.yaml` under `gitops/02-infra`, `gitops/03-finops`, and `gitops/04-business-app`.
...
- dashboards exist as code under `gitops/02-infra/observability-objects/dashboards/manifests/`,
