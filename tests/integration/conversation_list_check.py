...
из пяти, а не редкое совпадение. Ровно поэтому одиночного курсора мало,
...
async def run() -> None:
    ...
    async with pool.acquire() as conn:
        ...
        try:
            ...
            # Одиночный курсор остаётся законным, и его слабость объявлена
            # контрактом. Здесь она показана прямо: на отметке группы равных
            # он отдаёт пустую страницу, потому что строгое `<` по одной
            # колонке вычитает всю группу целиком. Это не дефект проверки
            # и не довод «укрепить» параметр отказом — довод в том, что
            # собственный `next_*` всегда отдаёт пару.
            alone = await page(ActivityCursor(updated_at=tie_mark))
            check(
                "одиночный курсор на группе равных отдаёт пустую страницу",
                alone.ok and not alone.page.items,
                str(ids(alone.page) if alone.ok else alone.rejection),
            )
