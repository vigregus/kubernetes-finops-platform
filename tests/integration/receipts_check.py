...
async def run_race_a(
    ...
    Ответы устройств совпадать **не** обязаны. Устройство, применившееся
    первым, отражает состояние после собственной точки сериализации — второе
    к тому моменту ещё не применено, — и требовать от него чужого будущего
    значило бы проверять не то, что сервер обещает. Обязательно другое:
    итоговая строка после обоих обновлений равна покомпонентному максимуму
    присланных состояний. Именно это и есть потерянное обновление, ради
    которого гонка заведена: присваивание вместо максимума сохранило бы
    чужой номер, и строка оказалась бы ниже.
    """
    async with (
