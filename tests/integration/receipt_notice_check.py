...
**Маска видна и в теле беседы — с обеих сторон.** Канал глушится целиком,
поэтому из одной тишины не следует, что маскируется **тело**: второй
транспорт маску обходил бы. Здесь проверяется и оно — у того, с кем зритель
в блокировке, в ответе списка нет ни `online`, ни `last_seen_at`, а его
запись `read_states` отсутствует; своя запись при этом остаётся, и имя
участника тоже — скрывается активность, а не человек (`BLK-004`).
Скрытие выражается **отсутствием**, а не `false`: «не в сети» — это
утверждение сервера, а маска обязана не утверждать ничего. Проверка идёт
парой — то же самое чтение до строки в `blocks` — иначе отсутствие ключа
неотличимо от поля, которого сервер не отдаёт вовсе.

Тело беседы читается только **списком**. Второй маршрут (`POST
/conversations`) при блокировке не отдаёт тела совсем: `authorize` отказывает
раньше сборки ответа (`creation_blocked_between`), и это проверяется
отдельным утверждением — иначе «второй маршрут не проверен» читалось бы как
«второй маршрут течёт». Сборка тела при этом общая на оба маршрута, и её
половина закрыта юнитом, где предикат подставлен (`test_presence_endpoints`).
...
async def conversation_from_list(
    branch: httpx.AsyncClient,
    *,
    conversation_id: str,
    headers: dict[str, str],
) -> dict:
    """Тело беседы из `GET /conversations` — как пришло, без разбора.

    Возвращается элемент целиком, а не половина его: квитанции и присутствие
    приходят одним ответом, и вторая половина нужна там же, где первая, —
    у маски (срез 3) предметом служат обе. Разбирать ответ дважды, двумя
    запросами, значило бы позволить им разойтись между собой.
    """
...
async def states_from_list(
    ...
    *,
    ...
    return _by_user(
        (await conversation_from_list(
            branch, conversation_id=conversation_id, headers=headers
        )).get("read_states", [])
    )
...
async def check_activity_visibility(
    branch: httpx.AsyncClient,
    *,
    conversation_id: str,
    ...
    """Что каждый из двоих читает про активность другого — в одном из двух состояний.

    Одна функция на обе половины пары, потому что различие между ними ровно
    одно — строка в `blocks` (тот же приём, что у тишины канала выше): при
    `masked=False` она проверяет, что активность **видна**, при `masked=True` —
    что её нет. Без первой половины вторая ничего не доказывает: «ключа нет»
    одинаково выглядит и у маски, и у поля, которого сервер не отдаёт вовсе,
    и у ответа без участника.

    Проверок на каждого зрителя три, и они разные. Присутствие — по обоим
    ключам сразу (`online` и `last_seen_at`): маска обязана снять оба, и
    снятый наполовину ответ прошёл бы на одном. Но «видно» доказывается
    только `online`: `last_seen_at` отсутствует и законно — у человека,
    который ни разу не подтверждался, отметки нет, и требовать её значило бы
    требовать чужого факта. Квитанции — раздельно: чужая маскируется, своя
    остаётся, и на равных нулях подмена «чужое скрыто» на «своё потеряно»
    прошла бы незамеченной. Имя участника — положительный контроль: ответ,
    из которого человека выбросили целиком, маской не является (`BLK-004`:
    старая переписка остаётся видимой).

    Только список, а не оба REST-маршрута, и это измеренное ограничение, а не
    забывчивость: `POST /conversations` при блокировке не отдаёт тела вовсе
    — `authorize(CREATE_CONVERSATION)` отказывает раньше сборки
    (`creation_blocked_between`, `BLK-001`), — поэтому маскировать в его
    ответе нечего, и проверка ниже это утверждает. Вторая точка сборки тела
    при этом покрыта: `_conversation_body` получает `blocked_with` на обоих
    маршрутах, и это проверено юнитом, где предикат подставлен.
    """
    for зритель, имя_зрителя, чужой, имя_чужого, свой in (
        (headers_a, "A", ид_b, "B", ид_a),
        (headers_b, "B", ид_a, "A", ид_b),
    ):
        тело = await conversation_from_list(
            branch, conversation_id=conversation_id, headers=зритель
        )
        участники = _by_user(тело["participants"])
        квитанции = _by_user(тело.get("read_states", []))
        чужой_участник = участники.get(str(чужой))
        скрыто = (
            чужой_участник is not None
            and "online" not in чужой_участник
            and "last_seen_at" not in чужой_участник
        )
        видно = чужой_участник is not None and "online" in чужой_участник
        check(
            f"{label}: {имя_чужого} остаётся в ответе для {имя_зрителя}",
            чужой_участник is not None and bool(чужой_участник.get("display_name")),
            str(чужой_участник),
        )
        check(
            f"{label}: активность {имя_чужого} "
            f"{'скрыта от' if masked else 'видна'} {имя_зрителя}",
            скрыто if masked else видно,
            f"участник: {чужой_участник}",
        )
        check(
            f"{label}: чужая квитанция {'скрыта от' if masked else 'видна'} {имя_зрителя}",
            (str(чужой) in квитанции) is not masked,
            str(квитанции),
        )
        check(
            f"{label}: своя квитанция {имя_зрителя} остаётся на месте",
            str(свой) in квитанции,
            str(квитанции),
        )
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                for имя, блокирующий, заблокированный in (
                    ...
                    # Первая половина пары — до строки в `blocks`: активность
                    # собеседника в REST видна обоим. Вторая — та же самая
                    # проверка при блокировке, и различие ровно одно.
                    await check_activity_visibility(
                        branch,
                        conversation_id=беседа,
                        headers_a=headers_a,
                        headers_b=headers_b,
                    ...
                    await check_activity_visibility(
                        ...
                        ид_a=ид_a,
                        ид_b=ид_b,
                        label=f"{имя}, при блокировке",
                        masked=True,
                    )
                    # Второй маршрут при блокировке тела не отдаёт вовсе —
                    # отказано **до** сборки ответа (`creation_blocked_between`
                    # в `authorize`), и это сильнее маскировки: маскировать
                    # поле в теле, которого нет, нечего. Утверждение стоит
                    # здесь затем, чтобы «второй маршрут не проверен» нельзя
                    # было прочитать как «второй маршрут течёт»: он не течёт
                    # — он не отвечает.
                    переиспользование = await branch.post(
                        "/conversations",
                        json={"participant_id": str(ид_b)},
                        headers=headers_a,
                    )
                    check(
                        f"{имя}: при блокировке второй маршрут тела не отдаёт",
                        переиспользование.status_code == 403
                        and "participants" not in переиспользование.text,
                        f"{переиспользование.status_code}: {переиспользование.text[:200]}",
                    )
                    await pool.execute(
