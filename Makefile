...
web-check: ## Веб: клиент из контракта, сборка, линтер, модульные тесты
	...
	@# Статический гейт: граф импортов от `main.tsx` не достигает фикстур и
	@# закрытого списка B15. Здесь, а не в `npm test`, потому что проверяется
	@# **достижимость** модуля от точки входа, а не поведение компонента:
	@# `ConnectionStateBanner` при `connected` рендерит `null`, и текстом его
	...
	@# Здесь, а не в `web-check`-тестах: проверяются две стороны сразу —
	@# собранное дерево и рендер чарта, — и ни одна из них не видна из
	@# модульного прогона. Образ собирается, чарт рендерится, пробы отвечают,
	@# а `DEP-001` при этом нарушен: имя источника идентичности вшито в бандл,
	@# и второй стенд требует второй сборки. Ни один существующий детектор
	@# этого не видит — поэтому проверка своя.
	@echo "· веб: один образ на оба окружения (DEP-001)"
	@python3 scripts/check-runtime-config.py
