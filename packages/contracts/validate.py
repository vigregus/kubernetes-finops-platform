...
def _types(schema):
    """Множество объявленных типов.

    `None` — ограничение `type` не объявлено вовсе. Это не «нечего
    сравнивать»: схема без типа шире любой типизированной, поэтому
    появление `type` после такого состояния — сужение, и судить как раз
    есть о чём. Снятие типа, наоборот, ограничение снимает.
    """
