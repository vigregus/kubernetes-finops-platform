...
def test_появление_типа_у_безтипового_параметра_видно():
    # «Тип не объявлен» — не «нечего сравнивать», а самое широкое из
    # возможных состояний: схема `{}` принимает любое значение, схема
    # `{type: string}` — только строку. У перечисления, формата и границ
    # симметричная ветка была, у типа её забыли, и `type` при этом назван
    # в `COMPARED_KEYS` — то есть проверка обещала его контролировать.
    findings = validate.parameter_schema_findings(
        {}, {}, _typed("p", {}), _typed("p", {"type": "string"}),
    )
    assert findings == ["появилось ограничение типа ['string']"], findings
