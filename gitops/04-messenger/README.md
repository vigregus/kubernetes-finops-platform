...
**Образы только по digest.** Чарт отказывается собраться с подвижным тегом:
...
`image.allowMutableTag` и действует только пока кода нет.

**Чего здесь нет.** Кода сервисов: сейчас под `api` — заглушка, которая
отвечает на пробы, чтобы каркас был проверен работой раньше, чем появится
первый настоящий образ. Namespace `messenger`
