 ...
 * модели на ходу. Несколько вложений в сообщении — дизайн G4 (`ATT-001…008`,
 * `IMPLEMENTATION-PLAN.md`), а не этого гейта: указатель здесь стоял на
 * `G3-007`, и второй элемент не появился бы и после него — отправки вложений из
 * браузера нет вовсе (композера в дереве нет, `check-web-purge.py`).
 *
 ...
 *   доедет подписью и **без** вложения; граница канала снимается тем гейтом,
 *   который приносит вложения в публикацию, — G4, и `G3-007` здесь стоял
 *   неверно: он квитанции и присутствие, а не вложения.
