 ...
 * приложение вообще касается окружения (`beginAndNavigate` в `session.ts`), —
 * поэтому тесты входа обязаны его предъявить. Именно предъявить: тест,
 * оставшийся без конфигурации, обязан упасть по имени, а не найти вшитое
 * значение. Обратное — снятие — делает `afterEach`, иначе отсутствие
...
/**
 * Хранилище со счётчиком записей. «Один вход» — это утверждение о числе
 * начатых входов, и читать его надо числом, а не догадкой о том, что было
 * последним: обе записи оставляют в хранилище ровно одно значение, поэтому
 * различить один вход от двух по содержимому нельзя.
 */
...
describe("адрес входа", () => {
  ...
  it("двойной клик по «Войти» — один вход, а не два", async () => {
    ...
    const [first, second] = await Promise.all([
      startLogin({ store, origin: ORIGIN, navigate }),
      startLogin({ store, origin: ORIGIN, navigate }),
    ]);

    // Между записью `pending` и уводом браузера стоит `await` — на
    // `crypto.subtle.digest`. Второй клик успевает записать **свой** `pending`
    // раньше, чем первый увёл браузер, поэтому входов оказывается два: в
    // хранилище остаётся `state` второго, а браузер уходит по адресу первого.
    // Вернувшийся из Keycloak `state` сверяется тогда с чужим `pending`, и
    // обмен отвергается как чужой — вход не удаётся по причине, которой
    // человек не делал.
    expect(store.writes).toBe(1);
    expect(visited).toHaveLength(1);
    expect(first).toBe(second);

    // Согласованность адреса и хранилища — второе утверждение того же
    // инварианта: уходит ровно тот `state`, который остался до возврата.
    const pending = consumePendingLogin(store);
    expect(new URL(visited[0]).searchParams.get("state")).toBe(pending?.state);
  });
});
...
describe("вход без конфигурации окружения", () => {
  ...
  it("неудачная попытка не запирает вход: следующая с конфигурацией проходит", async () => {
    ...
    // навсегда». `loginInFlight` обнуляется в `finally`, и это ровно то место,
    // где такая ошибка была бы видна, — повтор после неудачи.
    givenRuntimeConfig();
    const state = await startLogin({
      store,
      origin: ORIGIN,
