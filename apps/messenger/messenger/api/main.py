...
async def create_direct_conversation(
    ...
    return _conversation_body(
        result.conversation, result.participants, result.read_states
    )
...
async def list_conversations(
    ...
    Курсор принимается только целым. Параметры приходят по отдельности,
    поэтому «половина пары» — это единственное состояние, которое может
    собраться из строки запроса, и оно отвергается `400 invalid_cursor`
    до обращения к базе. Отвергается не ради строгости формы: запрос
    «строго старше отметки» на группе равных вычитает её целиком, то есть
    отдаёт **неполный список**. Пустая страница на месте отказа была бы
    хуже ошибки — она выглядит как конец списка, клиент на ней
    останавливается и о потере не узнаёт. Соблазн «оставить одиночную
    отметку законной, раз объявлена» здесь отвергнут сознательно:
    объявленный контракт закреплял не поведение клиента, которого нет
    (веб-клиент — G3-005), а известную потерю данных.
    ...
    # Курсор собирается целым или не собирается вовсе. Половину пары
    # отвергает проверка выше, поэтому здесь оба компонента есть вместе
    # или нет ни одного — третьего состояния у `ActivityCursor` нет,
    # и собрать его в обход проверки нечем.
    cursor = None
    if before_activity_at is not None and before_conversation_id is not None:
        cursor = ActivityCursor(
            updated_at=before_activity_at,
            conversation_id=ConversationId(before_conversation_id),
        )
...
def _conversation_body(
    ...
    *,
    ...
    `unread_count` отдаётся, когда проекция знает, и **восстанавливается**,
    когда не знает. До `G3-003` ключа не было вовсе — продюсера не
    существовало (`IMPLEMENTATION-PLAN.md:877-886`), — и это было верное
    решение того времени, а не временное. Теперь потребитель есть,
    а проекция — производная, и у производной есть источник истины:
    строку, которой нет, список восстанавливает из `messages` и
    `read_states` на самом чтении (`services/unread.restore_lost_counts`).
    Поэтому потеря проекции ответа не меняет — иначе потеря производной
    меняла бы существенный ответ системы, то есть проекция была бы не
    производной, а самостоятельным хранилищем.

    Ключ при этом остаётся необязательным, но означает теперь другое:
    «источник истины не даёт числа этому читателю» (он не в составе
    беседы), а не «проекция потеряна». Ноль на месте отсутствия всё так же
    недопустим: `0` — уверенный ответ «всё прочитано», и он не должен
    вставать на место ответа, которого нет.

    Считать его как `last_seq - last_read_seq` по-прежнему **нельзя**:
    так считались бы собственные сообщения, и полученное число не
    отвечало бы на вопрос «сколько я пропустил». Считаются чужие
    **позиции** `conversation_seq` выше `last_read_seq` — и именно поэтому
    нужна проекция: настоящий `COUNT` на каждый список бесед есть тот
    самый read amplification, ради устранения которого она и заведена.
...
def _conversation_list_body(
    ...
    намеренно — он был бы вторым описанием того же правила. Контракт
    поэтому объявляет оба поля обязательными (`ConversationListPage`),
    а не необязательными: сгенерированный клиент иначе получил бы
    `next_before_conversation_id?: string | null` и проверял бы наличие
    ключа там, где протокол обещает поле.

    Инвариант «либо оба `null`, либо оба непусты» держится построением,
    а не проверкой: `ActivityCursor` — пара целиком, поэтому взять
    `updated_at` из курсора, а идентификатор откуда-то ещё, нечем.

    `unread_count` передаётся как есть, вместе с `None`: различие
    «неизвестно» и «ноль» — часть ответа, и снять его здесь (подставив
    умолчание) значило бы потерять то, ради чего поле необязательно.
    """
    ...
    return {
        ...
        "next_before_conversation_id": (
            str(cursor.conversation_id) if cursor is not None else None
        ),
    }
