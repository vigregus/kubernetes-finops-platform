...
def _conversation_body(
    ...
    `unread_count` отдаётся, когда проекция знает, и **отсутствует**, когда
    не знает. До `G3-003` ключа не было вовсе — продюсера не существовало
    (`IMPLEMENTATION-PLAN.md:877-886`), — и это было верное решение того
    времени, а не временное. Теперь потребитель есть, проекция строится,
    и её отсутствие означает конкретную вещь: строки нет, то есть для этой
    пары (пользователь, беседа) событий ещё не применялось. Это `unknown`,
    а не ноль: `0` — уверенный ответ «всё прочитано», и показывать его
    вместо «не знаю» значило бы врать в ту сторону, где клиент перестанет
    показывать счётчик вообще.

    Считать его как `last_seq - last_read_seq` по-прежнему **нельзя**, и
    запрет не отменяется: так считались бы и собственные сообщения,
    и надгробия, — то есть число, не отвечающее на вопрос «сколько я
    пропустил». Именно поэтому проекция и заведена отдельной таблицей.
