...
class ConversationSummary:
    ...
    `read_states` **обязателен**, в отличие от обоих полей выше, и это не
    непоследовательность, а то же различие «отсутствие ≠ ноль», доведённое
    до конца. Необязательное `read_states = ()` дало бы **два неразличимых
    состояния**: «никто квитанций не присылал» и «мы их не загрузили».
    Проекция непрочитанного от этого защищена источником истины, а здесь
    источника нет: квитанция — не производная, а сам факт, и восстановить
    её не из чего. Обязательность делает `()` однозначным «спросили,
    квитанций нет»: конструктор у типа один (репозиторий списка), и он
    читает состояние всегда — вместе с участниками, тем же подзапросом.

    Разреженность при этом остаётся: пустой кортеж значит «никто не
    присылал», а не «у всех ноль», и элемент-ноль в нём появиться не может
    (`ParticipantReadState`). Наружу — в схеме беседы — поле, наоборот,
    **необязательное**: обязательное поле сломало бы совместимость
    со старым клиентом, и это то же ограничение, что у `online`
    (`CTR-003`). Требовательность типа и необязательность схемы отвечают
    на разные вопросы: первая — «знаем ли мы», вторая — «переживёт ли
    это старый клиент».
    """
