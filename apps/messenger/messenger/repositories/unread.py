...
`_unread_predicate` и подставляется в три запроса: пересчёт квитанции,
пересборка и (в виде отдельной ветки) чтение проекции списком. Тот же
предикат есть на Python в `domain/unread.counts_as_unread`, и тождество
этих двух половин не постулируется: интеграционная сверка `reconciliation`
доводит проекцию событиями, затем пересобирает её из источника истины
до того же чекпойнта и сравнивает числа поимённо. Разойтись они могут
только вместе с этим тестом.
...
(`unread_check.py`, сценарии `projection_deleted_*`).
...
def _unread_predicate(
    *, above: str, through: str, reader: str
    ...
    решение, а не пропуск: события удаления в потоке нет, поэтому
    исключение удалённых из пересчёта сделало бы пересборку строго
    меньше приращения. Разбор — в `domain/unread.counts_as_unread`.
    """
    return (
...
async def lock_offsets(
    ...
    Отсутствие строки `conversations` — единственная причина, по которой
    вставка может упасть: внешний ключ. Сегодня это недостижимо (факт
    публикуется только для сообщения, а сообщение ссылается на беседу
    внешним ключом, и удалять беседы в v1 нечем), и падение это громкое:
    цикл потребителя не зафиксирует смещение и запишет `unread_cycle`
    с `result="failed"`. Молчаливого расхождения не будет — будет
    вставший потребитель и видимая запись в журнале.
    """
    await conn.execute(
        """
    ...
    if value is None:
        raise RuntimeError("строка чекпойнта исчезла после вставки")
...
async def fetch_projection(
    ...
    *,
    ...
    Отсутствие беседы в ответе означает **неизвестно**, а не ноль, и
    наружу это различие выходит ключом: список бесед не отдаёт
    `unread_count`, которого не знает (`LIST-003` — тем же правилом,
    что `last_message`). Подставить сюда ноль значило бы показать
    пользователю уверенную ложь: «всё прочитано» вместо «проекция
    потеряна», причём ровно после её потери.
    """
