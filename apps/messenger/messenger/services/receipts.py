...
что вышли, уедет сообщение. Здесь транзакция не нужна **не потому, что
запись одна**, а потому, что нет границы согласованности, которую она могла
бы дать: голова монотонна, проверка границы односторонняя, а изменение
состояния — один `INSERT … ON CONFLICT DO UPDATE`, атомарный сам по себе.
Обёртка не добавила бы ни одной гарантии, зато создала бы ложное
впечатление: на READ COMMITTED каждый оператор транзакции получает
собственный снимок, поэтому чтение головы и запись в общей обёртке **не**
увидели бы согласованного состояния — того самого, ради которого обёртку
обычно и берут. Отсюда правило, которое стоит держать в голове: транзакция
там, где требуется атомарность **нескольких шагов**, а не «там, где записей
больше одной». Второе правило однажды применили бы к одиночному `UPDATE`,
обязанному быть атомарным относительно предшествующего `SELECT`.
"""
...
async def set_receipts(
    ...
    *,
    ...
    лишь до тех пор, пока всякий участник вправе прочитать всё. У бывшего
    участника и при блокировке доступ уже частичный, и граница для них
    считается иначе; условие это того, кто вводит эти состояния, — сегодня
    ни `left_at`, ни блокировка на чтение ещё не влияют.
