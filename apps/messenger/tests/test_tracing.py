...
def test_дерево_потребителя_и_ссылка_на_отправителя(экспортёр, monkeypatch):
    ...
    спаны = записанные(экспортёр)
    # Корень трассы — вся итерация цикла (`consumer-realtime-cycle`), а не
    # сама пачка: PING в Redis (`cache.healthy()`) выполняется до
    # обработки, инструментированным клиентом, и без span'а-обёртки
    # заводил бы собственную несвязанную трассу на каждую итерацию —
    # 159 таких трасс-сирот на 1000 сохранённых были замерены на живом
    # Tempo (repositories/postgres.py несёт тот же приём для asyncpg).
    # Пачка (`consumer-realtime`) при этом остаётся тем узлом, где решает
    # хвостовая выборка: `decision_wait` коллектора считается от него же,
    # просто он теперь не корень, а прямой потомок.
    assert len([span for span in спаны if span.name == "consumer-realtime"]) == 1
    цикл = по_именам(спаны)["consumer-realtime-cycle"]
    assert цикл.parent is None

    корень = по_именам(спаны)["consumer-realtime"]
    assert родитель(корень) == участок(цикл)
