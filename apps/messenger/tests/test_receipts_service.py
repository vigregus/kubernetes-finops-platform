...
def _projection(
    monkeypatch: pytest.MonkeyPatch, *, watermark: int = 0, count: int = 0
) -> Projection:
...
def test_пропавшая_беседа_не_занимает_замок(monkeypatch):
    ...
    Строка `unread_offsets` ссылается на беседу внешним ключом, и её
    вставка на несуществующей беседе упала бы **раньше** проверки
    `head is None`: честный `404` стал бы сбоем обработчика. Поэтому
    голова читается до транзакции, и здесь это видно как ноль открытых
    транзакций — то есть замок не просто отпущен, он не брался.
    """
    conn = Connection()
    _allow(monkeypatch)
    _head(monkeypatch, None)
