...
def _запись(*bodies: dict) -> list[tuple[str, dict[str, str], dict]]:
    """Пачка из Kafka: топик, заголовки, тело."""
    return [(FACT, {}, body) for body in bodies]
...
def test_пачка_даёт_один_замер_и_одну_фиксацию(monkeypatch):
    """Ронит замер внутри цикла по событиям.
