...
def test_отказ_несёт_ту_же_трассу_в_теле_и_в_заголовке(client, отказ):
    """Тело отказа и заголовок указывают на одну запись в журнале.

    В поддержку приходят со снимком экрана, а не с заголовками ответа,
    поэтому трасса кладётся и в тело. Разойдись эти два значения — поиск
    по журналу приводил бы к чужому запросу, и это выглядело бы
    как найденный ответ, а не как промах.
    """
    чужая = "4bf92f3577b34da6a3ce929d0e0e4736"
    r = client.get("/sessions", headers={"traceparent": f"00-{чужая}-00f067aa0ba902b7-01"})
    body = отказ(r, status=401, code="unauthenticated")
    assert body["trace_id"] == чужая
