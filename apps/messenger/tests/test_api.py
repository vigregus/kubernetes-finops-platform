"""Проверки проводки API: пробы, метрики, журнал обращений.

Бизнес-логики ещё нет, поэтому проверяется ровно то, что есть: сервис
отвечает на пробы, отдаёт метрики, не создаёт временных рядов на каждый
путь и пишет журнал в общем конверте.
"""
...
def test_готовность_отражает_отсутствие_проверок(client):
    """Пока проверять нечего, ответ говорит это явно, а не «всё хорошо»."""
    r = client.get("/readyz")
    body = r.json()
    assert "checks" in body
    # Пустой список проверок не должен выглядеть как успешная проверка.
