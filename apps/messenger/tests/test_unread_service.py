...
# Беседа, удалённая вместе со своими производными: ни строки проекции,
# ни строки чекпойнта — каскад `0011` уносит обе.
GONE = ConversationId(uuid.UUID("88888888-8888-8888-8888-888888888888"))
...
def _stand(
    ...
    *,
    ...
    async def _lock(conn_: Connection, *, conversation_id: ConversationId):
        ...
        # `gone` — это живой ответ `lock_offsets`, а не подстановка:
        # у удалённой беседы чекпойнта не остаётся (каскад `0011`),
        # поэтому «строки нет» и «беседы нет» — одно и то же.
        return None if gone else ConversationSeq(watermark)
...
# ---------------------------------------------------------------------------
# Беседа исчезла, а событие осталось
# ---------------------------------------------------------------------------


def test_событие_удалённой_беседы_пропускается_а_не_роняет_пачку(monkeypatch):
    """Ронит падение на беседе, которой уже нет.

    Живой прогон: уборка интеграционной проверки удалила беседу, а её
    события в потоке остались. Вставка чекпойнта падала внешним ключом,
    смещение не фиксировалось, `rewind` возвращал пачку в поток — и так
    **пятнадцать часов и 749 отказов**, ни одного применённого события
    у всех бесед этой партиции. Разовое падение переживают, вечное — нет:
    очередь за одной удалённой беседой не двигается больше никогда.

    Поэтому ответ — исход, а не исключение. И «пропустить» здесь значит
    трогать **ничего**: не приращивать (приращивать некому — проекции
    удалённой беседы тоже нет), не пересобирать, не двигать чекпойнт
    (двигать его нечем, строки нет). Проверяется это журналом целиком,
    а не одним полем: исход без записи был бы неотличим от «применил
    и промолчал».
    """
    conn, stand = _stand(monkeypatch, gone=True)

    outcome = _apply(conn, conversation_seq=5)

    assert outcome.kind is UnreadOutcomeKind.IGNORED
    assert outcome.reason == service.REASON_CONVERSATION_GONE
    assert conn.calls == ["transaction", "lock_offsets"]
    assert stand.deltas == [] and stand.offsets == [] and stand.rebuilds == []


def test_пропуск_беседы_отличим_от_чужого_типа(monkeypatch):
    """Ронит один код на два происшествия.

    `IGNORED` без причины — это штатный чужой тип события, он сыплется
    в журнал постоянно и означает «событие не про непрочитанное»; тот же
    исход с причиной `conversation_gone` означает удаление беседы, чьи
    события ещё живы. Оператор обязан видеть их врозь: искать по журналу
    «сколько мы пропустили из-за удалённых бесед» можно, только если
    второй случай во что-то выделен.
    """
    conn, _ = _stand(monkeypatch, gone=True)
    gone = _apply(conn, conversation_seq=5)
    ...
    assert gone.kind is foreign.kind is UnreadOutcomeKind.IGNORED
    assert gone.reason is not None and foreign.reason is None
...
def _restore_stand(
    ...
    *,
    ...
    Беседа, которой нет в `watermarks`, отвечает `None` и от замка: у неё
    нет строки чекпойнта, а у удалённой беседы её и не остаётся — каскад
    `0011` уносит её вместе с беседой. Стенд повторяет это как есть, чтобы
    недостижимое состояние (строка чекпойнта есть, беседы нет) не
    оказалось бы здесь достижимым и не сделало бы следующую проверку
    зелёной на невозможном входе.
    """
    conn = Connection()
    stand = RestoreStand()
    kept = present or {}
    ...
    async def _lock(conn_: Connection, *, conversation_id: ConversationId):
        conn_.calls.append("lock_offsets")
        if conversation_id not in marks:
            return None
...
def test_отсутствие_числа_у_источника_истины_не_становится_нулём(monkeypatch):
    ...
    не отличит от правды; отсутствие ключа — «спросить не у кого». Случаев
    отсутствия два — читателя нет в составе и беседы нет вовсе, — и потеря
    проекции не приводит ни к одному из них: её чинит восстановление.
    Восстановление при этом **состоялось** — иначе проверка доказывала бы
    пропуск ремонта, а не ответ источника.
    """
    conn, stand = _restore_stand(
        monkeypatch,
        watermarks={OTHER: 4},
...
def test_удалённая_беседа_не_роняет_чтение_списка(monkeypatch):
    """Ронит падение на беседе, которой уже нет.

    Список показывает беседу, а между его сборкой и нашим вопросом её
    удаляет уборка. Второй такой же повод — потеря проекции у беседы,
    которую удалили раньше и чьи события ещё в потоке. Раньше здесь
    встала бы ошибка ключа внешнего ключа (`lock_offsets` заводил строку
    чекпойнта беседе, которой нет), то есть `500` на `GET` списка —
    и `500` на каждой странице, где такая беседа есть, пока её не уберут
    из выборки.

    Ответ тот же, что и «источник истины не даёт числа»: ключа нет.
    Проверяется не только отсутствие ключа, но и то, что соседняя беседа
    страницы обслужена: пропуск одной не имеет права уносить остальные.
    """
    conn, stand = _restore_stand(
        monkeypatch,
        watermarks={OTHER: 4},
    ...
    counts = _restore(conn, [GONE, OTHER])

    assert GONE not in counts
    assert counts == {OTHER: 3}
    # Удалённую не пересобирают и не спрашивают у неё голову: пересборка
    # по беседе, которой нет, вернула бы пусто, а запись чекпойнта —
    # ошибку ключа. Ремонт обязан кончиться на замке.
    assert stand.rebuilds == [{"conversation_id": OTHER, "through_seq": 4, "user_ids": (ANYA,)}]
    assert stand.offsets == []


def test_голова_исчезла_под_замком_чекпойнта_и_это_громко(monkeypatch):
    """Ронит молчаливое восстановление не того числа.

    Отсутствие головы у беседы, чей чекпойнт мы **держим** `FOR UPDATE`,
    невозможно: каскадное удаление беседы пошло бы через эту строку
    и дождалось бы нас. Но невозможное состояние обязано быть громким:
    ответ, посчитанный не по тому, чего нет, от верного неотличим,
    и заметить его было бы нечем. Отсюда и страж: он ловит не гонку,
    а собственное расхождение кода с этой моделью замков.

    Имя названо по тому, что здесь происходит, а не по тому, чего здесь
    случиться не может: беседу под замком не удаляют, под замком
    пропадает **голова** — единственное, чего страж и касается. Про
    исчезнувшую беседу — `test_удалённая_беседа_не_роняет_чтение_списка`
    рядом и `test_событие_удалённой_беседы_пропускается_а_не_роняет_пачку`
    выше.
    """
