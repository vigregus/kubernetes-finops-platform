...
required = [
    ...
    root / "tests/e2e/g3-007-web.spec.ts",
...
g3_007 = root / "tests/e2e/g3-007-web.spec.ts"
g3_007_code = "\n".join(code_by_file.get(g3_007, []))

if "test(" not in g3_007_code:
    violations.append("tests/e2e/g3-007-web.spec.ts: в спеке нет ни одного теста")

# Каждая нога гейта читается со своей production-поверхности. Нога, у которой
# поверхности нет, проверяет не то, что названа: счётчик строки списка и номер
# прочтения собеседника — разные вещи, и подменять один другим значило бы
# доказывать одно через другое.
for surface, leg in (
    ("data-unread-count", "непрочитанное в списке бесед"),
    ("data-peer-read-seq", "номер прочтения собеседника"),
    ("data-presence", "присутствие в шапке"),
):
    if surface not in g3_007_code:
        violations.append(
            f"tests/e2e/g3-007-web.spec.ts: нога «{leg}» не читается с "
            f"production-поверхности (`{surface}`)"
        )

# Квитанция отправляется полем, объявленным контрактом. Проверяется и поле, и
# маршрут: `read_seq` без запроса — это строка в разметке, а не квитанция.
if "receipts" not in g3_007_code or "read_seq" not in g3_007_code:
    violations.append(
        "tests/e2e/g3-007-web.spec.ts: квитанция не отправляется "
        "(`POST …/receipts` полем `read_seq`) — тогда вперёд идти нечему"
    )

# Входов ровно **по одному на сценарий** — то есть по одному на человека.
# Лишний означал бы, что вторая вкладка ходит своим входом: это уже две сессии
# и два устройства, а не «одна сессия живёт сразу в нескольких вкладках»
# (`0006_realtime_connections.sql`). Проверка считается от числа тестов, а не
# вписана числом: иначе она покраснела бы на добавленном сценарии, ничего не
# нарушившем, и её сняли бы — вместе с тем, что она держит.
tests_007 = len(re.findall(r"^test\(", g3_007_code, re.M))
logins = g3_007_code.count("signIn(browser,")
if logins != tests_007:
    violations.append(
        f"tests/e2e/g3-007-web.spec.ts: входов {logins} на {tests_007} сценариев — "
        f"вторая вкладка обязана входить тем же контекстом (`signInTab`), "
        f"а не вторым входом"
    )

# Вторая вкладка — **страница того же контекста**: `addInitScript` кладёт
# `device_id` контексту, и страница его наследует. Свой контекст дал бы второе
# устройство, и `RCP-002` проверялся бы на другом предмете.
if ".newPage()" not in g3_007_code:
    violations.append(
        "tests/e2e/g3-007-web.spec.ts: вторая вкладка не заводится — "
        "сценарию нужны две страницы одного человека"
    )
if "signInTab(" not in g3_007_code:
    violations.append(
        "tests/e2e/g3-007-web.spec.ts: вторая вкладка не входит — токен живёт в "
        "памяти страницы (ADR 0005), и без входа её запросы нечем удостоверить"
    )

# `setOffline` гасит **весь** контекст: он обязан называть контекст стороны, а
# не приложение и не общий контекст на двоих. Общий погасил бы свидетеля вместе
# с предметом, и сверка при возврате доказывала бы пустоту.
offline_lines_007 = [line for line in g3_007_code.splitlines() if "setOffline(" in line]
if not offline_lines_007:
    violations.append(
        "tests/e2e/g3-007-web.spec.ts: уход в офлайн не предъявляется вовсе — "
        "без него у сверки при возврате нет повода"
    )
for line in offline_lines_007:
    if "contextA" not in line and "contextB" not in line:
        violations.append(
            f"tests/e2e/g3-007-web.spec.ts: setOffline вызван не на контексте "
            f"стороны (contextA/contextB) — свидетель и предмет делят контекст: {line.strip()}"
        )

# Фоновая нога снята **измерением**, а не тишиной. Мерка: после
# `bringToFront(B)` обе страницы одного контекста докладывают `visible`, и ни
# одного `visibilitychange` не приходит — в headless, в headed и со снятым
# `--disable-backgrounding-occluded-windows`. Приём, вернувшись в спеку, вернул
# бы и утверждение о фоне, которого браузер не предъявляет; нога живёт юнитом
# (`useReceipts`), и это названо в докстринге спеки.
if "bringToFront" in g3_007_code or "visibilityState" in g3_007_code:
    violations.append(
        "tests/e2e/g3-007-web.spec.ts: вернулась видимость вкладки — "
        "измерено, что headless-браузер её не подаёт (обе страницы `visible`, "
        "событий ноль); нога о фоне остаётся юнитом, а не тишиной в спеке"
    )
...
if [ "$FIXTURE_ONLY" = "1" ]; then
    ...
    # Аргументы сюда не пропускаются: цель диагностическая, и выбор проектов в ней
    # уже сделан.
    set -- test --project=fixture
else
    step "приёмка: playwright test — дочерний процесс этого shell"
    # Входящие аргументы **дописываются**, а не затираются. `set -- test` без них
    # молча отбрасывал бы всё переданное снаружи: узкий прогон
    # (`… --project=g3-007`) шёл бы полным, и «я гонял только свои сценарии» было
    # бы убеждением, а не фактом — проверено на себе. Инвариант предполётной
    # проверки (`playwright" "$@"`) обещает именно передачу аргументов.
    set -- test "$@"
fi
