...
def check_artifact(dist: Path, deployed_issuer: str, runtime_url: str | None) -> list[str]:
    ...
    if (dist / RUNTIME_FILE).exists():
        violations.append(
            f"в артефакте есть `{RUNTIME_FILE}` — файл обязан приходить из ConfigMap "
            f"окружения, а не из образа: вшитый, он становится значением по умолчанию, "
            f"и подстановка тома его не отменяет — она его прячет"
        )
...
def main() -> int:
    ...
    if not args.skip_artifact:
        checked.append(
            ...
            f"`{RUNTIME_FILE}`, а чтение `{GLOBAL_KEY}` есть"
        )
