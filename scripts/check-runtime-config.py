...
одна, но смотрит она с двух сторон.
  ...
  * стык страницы, nginx и чарта — адрес файла конфигурации объявлен один раз
    ...
    * у этого отказа есть **вторая дверь**, и видом тега она не закрывается:
      конфигурацию может прочитать сама точка входа — вызовом на верхнем
      уровне тела модуля. Тег при этом верен, читатель конфигурации в
      артефакте есть, сборка и типы сходятся, а последствие то же самое:
      исключение уходит из тела модуля до первого рендера, и страница не
      показывается вовсе. Поэтому место вызова проверяется отдельно и по
      **исходнику**, а не по бандлу: после минификации от тела модуля не
      остаётся ничего читаемого.
...
# Точка входа приложения. Читается по исходнику, а не по собранному бандлу:
# после минификации от тела модуля не остаётся ничего читаемого. Нужна здесь
# потому, что у отказа «конфигурации ещё нет» **две** двери, и видом тега
# (`loader_violations`) закрывается только первая: конфигурацию может
# прочитать и сама точка входа — вызовом на верхнем уровне тела модуля.
WEB_ENTRY = ROOT / "apps/web/src/main.tsx"

# Имя функции, которой читается конфигурация. Названо здесь, а не выведено из
# `runtime-config.ts`: проверка ищет **место** вызова, и вывод имени из
# проверяемого файла дал бы согласие с чем угодно — переименовали функцию, и
# правило перестало бы что-либо видеть.
CONFIG_LOADER = "loadRuntimeConfig("
...
def lazy_config_read(entry: Path) -> list[str]:
    """Точка входа читает конфигурацию **в момент нужды**, а не при загрузке.

    Вторая дверь в тот же отказ, что у `loader_violations`, и видом тега она
    не закрывается. Тег держит **порядок**: конфигурация подключается обычным
    скриптом и потому исполняется при разборе, а модуль приложения — после
    него. Но точка входа исполняется **внутри** этого модуля, и вызов
    `loadRuntimeConfig()` на верхнем уровне её тела случается ровно в тот же
    момент, когда конфигурации ещё нет, — с верным тегом, с зелёным
    `has_reader` в артефакте и с зелёной сборкой. Последствие при этом то же
    самое, и оно полное: не «realtime не поднялся», а страница не показана
    вовсе — исключение уходит из тела модуля до первого рендера.

    Найдено чтением, а не догадкой. Дефект внесён в `main.tsx`
    (`const centrifugoUrl = loadRuntimeConfig().centrifugoUrl`) и предъявлен
    зелёным у **всех** сторожей сразу: этот чек, `check-web-purge.py` (граф
    импортов не меняется — 72 модуля, ни одного запрещённого), `tsc -b` (типы
    сходятся) и модульные тесты (283) — `main.tsx` не импортируется ни одним
    из них, и прогонять его нельзя: он монтирует корень и уходит в сеть.
    Молчание всех четырёх и есть причина этой проверки.

    Ленивость выражается стрелкой или `return`: `() => loadRuntimeConfig()`
    внутри `readCentrifugoUrl`, ровно как читает себя `oidcIssuer`
    (`runtime-config.ts:83-86`). Строка верхнего уровня не содержит ни того,
    ни другого, и это единственное, что здесь решается без разбора
    TypeScript. Строки комментариев пропускаются: доккомментарий рядом
    называет функцию по имени, и без этого он краснел бы сам на себе.

    Ошибка возможна только в сторону «пропустить» — вызов, разнесённый по
    строкам, проверка не увидит. Ложных срабатываний нет по построению, и
    выбрано это намеренно: красный на верном коде обесценил бы проверку
    быстрее, чем пропущенный случай.
    """
    rel = shown(entry)
    if not entry.is_file():
        return [
            f"нет `{rel}` — точки входа, читающей конфигурацию, не найти: "
            f"«читает лениво» было бы верно по пустоте"
        ]

    violations = []
    for number, line in enumerate(entry.read_text(encoding="utf-8").splitlines(), 1):
        call = line.find(CONFIG_LOADER)
        if call < 0:
            continue
        stripped = line.strip()
        if stripped.startswith(("*", "//", "/*")):
            continue
        if "=>" in line[:call] or "return" in line[:call]:
            continue
        violations.append(
            f"`{rel}:{number}` читает конфигурацию при загрузке модуля: "
            f"`{stripped}`. Точка входа исполняется, пока конфигурации ещё нет "
            f"— файл подключается обычным скриптом **ниже** модуля приложения, "
            f"— поэтому чтение обязано быть отложено до момента нужды: "
            f"`() => loadRuntimeConfig()`. Вид тега при этом верен, и без этой "
            f"проверки отказ не видит никто: страница не показывается вовсе, "
            f"потому что исключение уходит из тела модуля до первого рендера"
        )
    return violations
...
def main() -> int:
    ...
    # Вторая дверь в отказ «конфигурации ещё нет»: вид тега верен, а точка
    # входа читает конфигурацию сама. Стоит **вне** половины «артефакт»: это
    # свойство исходника, и `--skip-artifact` его не отменяет.
    violations += lazy_config_read(WEB_ENTRY)
    ...
    checked = [
        ...
        f"точка входа `{shown(WEB_ENTRY)}` читает конфигурацию в момент нужды, "
        f"а не при загрузке модуля",
    ]
    if not args.skip_artifact:
        checked.append(
